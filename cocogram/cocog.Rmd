...
---
# NLP (modularized) pipeline

管線的發展讓我們可以把語言切整乾淨，方便從不同角度抽取觀察。
...
中文（與一些語言）多了一組斷詞任務，很強勢地決定了任何後續的任務。
...
<p align="center">
<img src="images/fiction.png" alt="drawing" width="700"/>
</p>



---
# 回去看看 NLP (modularized) pipeline 的預設


### 每個模組都要提供最佳解，贏者全拿 (competition) <br>
Resolution of any competition (always looking for a **best-fit** solution

  - 注意 competition 的源頭是來自連結論傳統 <br>
The notion of compitition is sometimes associted with highly *non-modular* processing theories also described as **connectionist**, which hypothesizes an unrestrcted flow of information across an uncompartmentalized mind.
        

### 語言（語法）模組是自給自足的自主 (autonomous) 系統
...
<p align="center">
<img src="images/chomsky.png" alt="drawing" width="500"/>
</p>
...
<p align="center">
<img src="images/sem.proc.png" alt="drawing" width="700"/>
</p>
...
<br>
<br>
## Grammar 是在人際使用中，系統裡突現的 (emerged)，(可能)不是先驗內在的 (innate)。
...
.font90[
a speaker's knowledge of language consists of a very large inventory of constructions, where a construction is understood to be $\underline{of \ any \ size \ and \ abstractness}$, ranging from morphemes to argument structure, and include obvious examples like <font color="blue">
**collocations** (`hermetically sealed'), **(idiomatic) expressions with fixed constituents** ('kick the bucket'), **expressions with (semi-) optional constituents** ('hungry as a X'), and **sequences of grammatical categories** ([det][adj][noun]), as well as more complex constructions involving, e.g., the occurrence of **sentence composition features** (e.g. transitivity) or
adverbial types (e.g. spatial adverbials). </font>
]
...
`CONTRUCTICON` 簡言之：A construction can be defined as a **form-meaning pairing** (at all levels of descriptions) in which the components cannot entirely explain the meaning of the whole.
]
]
...
# Constructional Meaning | 意義是建構的 1/
...
> Grammar does not involve any transformational or derivational component. Semantics is associated directly with surface form
...
# Constructional Meaning | 意義是建構的 2/

---

- Semantics is based on speakers' `construals` of situations, not on objective truth conditions.

- Semantics, information structure, pragmatics are interrelated, and all played a role in linguistic functions, which are part of our overall conceptual system and not a separate modular component.
...
# (I) Form-Meaning Pairings | 「形意對」的符碼單位
---

> Constructions are defined to be conventional, learned form-function pairings at varying levels of complexity and abstraction.
...
- .font90[大部分的構式具有（不同程度的）「不可預測性」的特質。（如：【X 什麼 X】具有否定、斥責）。成分與構式之間的規約與語意激發關係值得探究。]


- .font90[情 (sentiment) 意(sense) 計算是協同發生的嗎？] （【 X 到一個 Y】）


---
layout: true

# (II) Syntax-lexicon continuum 1/2
---
...
# (II) Syntax-lexicon continuum 2/2
---

如果句法本身有意義，就是詞項。所以句法與詞彙界線模糊。
.font70[All constructions are part of a syntax-lexicon continuum. (Hoffmann and Trousdale, 2013)]


(1) word construction: `apple`  

(2) idiom construction: `X take Y for granted` - 'X doesn't value Y'

(3) comparative construction: `John is taller than you` - 'X is more Adj than Y'

(4) resultative construction: `She rocks the baby to sleep` - 'X causes Y to become Z by V-ing'.

***
--

(1) 是典型的 form-meaning pairing; (3)-(4) 比較 schematic，因為有 slots
...
# (III) Cognitive Organization 1/
---


.font70[訊息的習得、儲存、組織與表徵]
...
# (III) Cognitive Organization 2/
---

### Usage-based theory and Exemplar Representation of Construction

> The mental grammar of speakers is shaped by repeated exposure to specific utterances and in which domain-general cognitive processes such as categorization and cross-modal association play a role in the entrenchment of construction.

- Under this view, there would be no distinction between grammar and metagrammar, but all linguistic knowledge is instead viewed as **emergent** and constantly changing.

  .font70[語法語意語用都存在一起了]

---
layout: true
# (III) Cognitive Organization 3/

---


- C\*G claims that **constructions form a (default inheritance) network**
...
- (Kaschak and Glenberg 2000) suggest that the constructional pattern specifies a general scene and that the 'affordances' of particular objects are used to specify the scene in detail (cf. Pustejovsky's 1995: qualia structure).
...
- cognitive commitment: (a commitment to providing a characterization of general principles for language that accords to what is known about the mind and brain from other disciplines (Lakoff 1990)).
...
- computational commitment
  ...
  - data friendly
  -  `C*G` are currently enjoying considerable attention in linguistic research, but not gaining a (central) place in different kinds of computational linguistics applications++; (.font90[但情緒分析 **sentiment analysis** 與語言習得 **language acquisition** 可能是個關鍵的著力點。]) 


.footnote[++2010 ACL-workshop on Extracting and Using
Constructions in Computational Linguistics' 之後就沒了]
...
- <font color=#0000FF>introspective</font>: from a speaker's second-level consciousness

- <font color=#0000FF>observational</font>: from recordings of corpora of linguistic production in (noisy) naturalistic settings

- <font color=#0000FF>experimental</font>: from subject's behaviour in designed controlled experimental situations facing carefully developed experimental stimuli. 



---
# Observational approaches to C*G


- Frequencies of (co-occurrence)

- Conditional Probabilities (Undirectional)
...
---
# Measuring (Form-Meaning) Contingency in Corpus Linguistcs

- a variety of measures of `contingency` (called **associative measures**).

- relies on the `contingency table` based on collocational approaches from corpus linguistics.
...
- <font color=#0000FF>collexeme analysis</font> computes for *n* words how strongly these words are attracted to a slot in a construction.

- <font color=#0000FF>(multiple) distinctive collexeme analysis</font> computes for *n* words how strongly these words are attracted to two or more functionally similar constructions.

- <font color=#0000FF>co-varying collexeme analysis</font> (item-based and system-based) computes for *n* words in one slot of a construction how strongly these words are attracted to the *y* words in another slot of the same construction.


所有這些方法都提供 rankings of how much words and particular slots of constructions attract each other (viz. the bidirectional nature of these association measures.)
...
---
layout: true
# Multifactorial and Multivariate Approaches 1/
---


> Given (1) the obviously *multifaceted* nature of language and its relation to, or interaction with cognitive processing and (2) the complexity and noisiness of data obtained from corpora, it is often necessary to resort to statistical methods that can do better justice to the observed data.
...
---
layout: true
# Multifactorial and Multivariate Approaches 2/
---


怎麼量度 **idiomaticity** on the basis of 
  - experimental data (e.g., judgment data from subjects using the magnitude estimation method)
  - corpus data (by operationalizing different dimensions of idiomaticity, such as `collocational overlap` and idiomatic `variation parameters` (describing morphological and syntactic parameters of the idioms's use) 
...
---
layout: true
# Multifactorial and Multivariate Approaches 3/
---

How to identify **prototypical instances of constructions**?

- Use a linear discriminant analysis to determine which characteristics were good predictors of the constructional choices in the corpus data (Gries, 2003). Each corpus instance was assigned a discriminant score that reveals how good, or prototipical, an example of ... is.
...
- simulation 
  - connectionist model to test constructional priming (Chang et al. 2000)
  - Traceback approach (Dabrowska et al 2005; )
...
- `如何「辨識」構式？` `如何解決同一性`？<br>
<font color=#0000FF> Unsupervised learning of construction (type/tokens)</font>

- `如何同時表徵符碼與意義?` <br>
"The form and content are symbolically linked" (form-meaning pairs) but how can both to be co-represented?

- `有無自然邊界？如何調節在言談中的更新流動與變異？` <br>
are texts possible to be naturally chunked? reconcile varieties? roles in the understanding of the text/discourse? 

- `隱性構式知識本體如何紬繹與回饋？` <br>
how a 'provisional unit boundary' can be detected, the 'linear unit of meaning' (linear unit grammar) be acquired and latent hierarchy can be discerned in the linear string of word forms? 
...
# [1] 構式如何辨識 ｜ form identification
---


- 詞彙也許是相對穩定的構式，但從單純計量角度是矛盾的。<br>
corpus-based counting of occurrence and co-occurrence seems straightforward, but require tokenization (thus assumes its boundary decision)

- 靜態語料庫難以處理 dispersion 的問題  <br>
aggregating token frequencies can be misleading since tokens may exhibit very uneven distributions in a corpus (and real contexts).


> Frequency answers " how often does *x* happen?" whereas dispersion asks "in how many contexts will you encounter *x* at all?" (Ellis and Gries, 2015)
...
# [2] 構式如何表徵 | Representation of Constructions
---

`Feature structure`(via Attribute Value Matrices (AVM))

<img src="images/lexeme.png" align = "left" alt="drawing" width="400"/>


<img src="images/caused-motion-avm.png" align = "right" alt="drawing" width="700"/>



<br>
參考：[Features and Unification](https://www3.cs.stonybrook.edu/~ychoi/cse507/slides/08-unification.pdf)
...
# [3] 構式有語意理論嗎 | Constructional Semantics
---
...
# 構式語法如何表徵語意 | meaning representation

---

(Usage-based perspective)

- Meaning = semantics, pragmatics, discourse-functions, ...

- Mental meaning is **construal** (not truth semantics), and most prominent framework is **Frame semantics** (Fillmore).
  - encyclopedic knowledge involved.
  - or, part of mental meaning is Frame.

- So, constructions as <font color=#0000FF> construals of frames </font>
  - interaction of verbal frame (event type and participants) and **Argument Structure** (VAC).
...
---
# 在計算語言學界難被注意
The task of <font color=#0000FF>form-function contingency learning</font> has
mostly been overlooked in NLP.

- 難處不僅在於 form 的 type/token indentification，而在 meaning 的對應 (contingency of mapping) 是如何知道的。

- 一形多義 (polysemy; multiple related senses) 與一形多用 (polyfunctionality/polycategoriality) 造成的歧義問題 <br>
cues with multiple interpretations are hard to resolve
...
---
# 計算心理語言模型 
Baayen's LDA (2020).. associative learning


- **Associative learning over the language stream** allows language users to 'find structure in time' and thus to make predictions (Elman, 1990).
...
<p align="center">
<img src="images/fluid1.png" alt="drawing" width="900"/>
</p>
...
<p align="center">
<img src="images/fluid2.png" alt="drawing" width="900"/>
</p>
...
<font color=#0000FF>Diachronic machine learning?</font>

- 以歷史語料學習，逼近字典與成語典(四字格)。
...
- Croft, William A. & D. Alan Cruse (2006). Cognitive Linguistics. Cambridge: Cambridge University Press.
- Fazly, A. et al. (2009). Unsupervised Type and Token Identiﬁcation of Idiomatic Expressions.*computational linguistics 35:1*. 
...
- Huang, S.F.(2017). *Formosan languages and (radical) construction grammar*. Talk at GIL, NTU.
...
- Herbst, Thomas. (2018). *Is language a collostruction? A proposal for looking at collocations, valency, argument structure and other constructions*.

- Hoffmann and Trousdale (eds). 2013. *The Oxford Handbook of Construction Grammar*.
